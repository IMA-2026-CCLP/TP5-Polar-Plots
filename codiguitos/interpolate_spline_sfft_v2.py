...
Para cada frame temporal:
  ...
  1. Extraer un frame corto de cada micrófono (4096 muestras).
...
# ── Parámetros STFT ───────────────────────────────────────────────────────────
# Frame de 4096 muestras a 48kHz → 85 ms, Δf ≈ 11.7 Hz
# Buena resolución para voz cantada F4–F5 (350–700 Hz)
FRAME_SIZE  = 4096
...
def stft_interpolar(senales: dict,
    ...
    for i in range(n_frames):
        ...
        # ── 1 & 2. Extraer frame, aplicar ventana Hann y FFT ─────────────
        espectros_frame = {}
        ...
        # ── 4 & 5. IFFT + ventana de síntesis + overlap-add ──────────────
...
def procesar():
    ...
    for ang_fuente in angulos_medicion:
        ...
        if MIC_VEC_IZQ not in mics_disp or MIC_VEC_DER not in mics_disp:
            print(f"  [SKIP] ángulo {ang_fuente}°: falta mic_{MIC_VEC_IZQ} "
        ...
        print(f"  {ang_fuente:>4}°  |  {len(angulos_disp)} mics  |  "
