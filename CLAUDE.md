...
- **`.cclp`** (`core/session.py`, thin wrapper over `core/data_store.py`) — lightweight session: same NPZ content as the directivity `.npz` below (computed directivity, per-note directivity, plot config), **no audio**, plus `metadata['view']['session_ui']` — the *entire* `Bridge.state`, not just the Directividad controls — so reopening one restores every tab's controls exactly as they were. Since there's no raw tensor, a loaded `.cclp` cannot be reprocessed (no re-align/calibrate/re-detect-notes/recompute-with-other-bands) — it's read-only for the 4 plots, same as loading a directivity `.npz`. For that, use "Guardar/Cargar audio procesado (.npz)" instead (`ui/file_loader.py`'s `save_session`/`load_session`, `MicArray.save`/`from_tensor` — still full audio, can be reprocessed). Note segmentation masks are separate: `.msk` JSON files via `TabNotas.save_mask`/`load_mask` (Herramientas ▸ Guardar/Cargar máscara…).
- **directivity `.npz`** (`core/data_store.py`) — directivity-only results for exchange/inspection, schema documented at the top of that file. `save_results()` will use the first computed note as the "global" section if the user never computed directivity on "Todo el audio", so the file is always loadable even when only per-note directivity exists. It also stores `metadata['view']` (the Directividad controls in `_DIR_UI_KEYS` plus each plot's Propiedades via `TabDirectividad.get_view_config()`), and loading it (`MainWindow._load_polar_npz_file`) repopulates the four plots, the note selector and the controls **without audio or a `MicArray`** (`TabDirectividad._npz` serves per-note data). `.cclp` reuses this same loader with `full_ui=True`. Both writers open the file handle themselves before calling `np.savez_compressed` — passing a bare path string makes numpy silently force a `.npz` suffix (so a `.cclp` would get written as `foo.cclp.npz`).
