...
- **`.cclp`** (`core/session.py`, thin wrapper over `core/data_store.py`) — lightweight session: same NPZ content as the directivity `.npz` below (computed directivity, per-note directivity, plot config), **no audio**, plus `metadata['view']['session_ui']` — the *entire* `Bridge.state`, not just the Directividad controls — so reopening one restores every tab's controls exactly as they were. Since there's no raw tensor, a loaded `.cclp` cannot be reprocessed (no re-align/calibrate/re-detect-notes/recompute-with-other-bands) — it's read-only for the 4 plots, same as loading a directivity `.npz`. There is no "save raw audio tensor" feature in the GUI (deliberately removed) — to reprocess, reload the original WAV files. Note segmentation masks are separate: `.msk` JSON files via `TabNotas.save_mask`/`load_mask` (Herramientas ▸ Guardar/Cargar máscara…).
