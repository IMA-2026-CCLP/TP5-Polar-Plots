...
`ui/tab_directividad.py` implements four simultaneous, independently-configurable views — Superficie 3D, Esfera, Polar 2D, Espectro — docked in a nested `QMainWindow` grid inside `TabDirectividad`. Each mode maps to a view class in `_VIEW_CLASS_BY_MODE`: Polar 2D/Espectro are native pyqtgraph (`ui/polar2d_view.py`/`ui/spectrum_view.py`); Superficie 3D is native `pyqtgraph.opengl` (`ui/gl3d_view.py`, migrated off Plotly as a pilot — see below); Esfera is still `BalloonView` (`ui/balloon_view.py` + `plot/balloon.py`, Plotly inside `QWebEngineView`), pending the same migration if the 3D pilot holds up. All four expose the same public API subset (`set_data`/`set_band`/`set_colorscale`/`set_style`/`set_camera_view`/`export_image`/the `log` and `context_menu_requested` signals/`mapToGlobal`) so `_ViewSection`/`TabDirectividad` treat them polymorphically — swapping the class in `_VIEW_CLASS_BY_MODE` is the only wiring change a future migration needs.

**`ui/gl3d_view.py` (Superficie 3D, native)**: reuses the mesh math verbatim from `plot/balloon.py::_build_hemisphere_grid` (interpolation, circular smoothing, front/back seam averaging, zenith cap) — only how it's drawn changed, from a Plotly `mesh3d` trace to a `pyqtgraph.opengl.GLMeshItem` with per-vertex colors sampled from `COLORSCALES`. Camera presets (`_CAMERA_PRESETS` in that file) are the same eye/up vectors as `balloon_view.py`'s, converted to `GLViewWidget`'s `(elevation, azimuth, distance)` model — `GLViewWidget.cameraPosition()`'s formula gives the exact conversion. Known simplifications versus the Plotly version: no true orthographic projection (the "Isométrica" preset keeps the right angle but stays in perspective — pyqtgraph.opengl has no ortho toggle), and the axis-aligned box grid is replaced by latitude/longitude reference circles (styled by the same "Ejes / grilla" Propiedades controls) since the data is inherently spherical. Right-click uses a plain Qt `eventFilter` on `ContextMenu` events — no relay hack needed, unlike the Plotly views below. Export clones a hidden `GLViewWidget` positioned off-screen (`move()` to negative coordinates, **not** `WA_DontShowOnScreen` — a `QOpenGLWidget` needs a real native window to get a GL context) at the fixed physical size, rebuilds the same mesh/axis/grid items on it, and calls `grabFramebuffer()`.

- plotly.js 2.32.0 is vendored in `app/plot/vendor/` and loaded by relative `<script src>` with that folder as the `setHtml` base URL (`PLOTLY_DIR`), so the Esfera view works **offline** (it used to come from the CDN). `polar_analyzer.spec` bundles the folder. Don't swap it for the pip `plotly` bundle without checking: that one ships plotly.js 3.x, which drops some attributes this code uses (e.g. `titlefont`).
...
- Image export (all four views, `export_image`) re-renders each plot — never a screenshot — at a FIXED PHYSICAL SIZE in cm (default 16×10 cm; Opciones ▸ Gráficos ▸ Imágenes, stored in `QSettings` via `ui/export_utils.py`): pixels = cm/2.54 × DPI, so changing the DPI only changes quality, not size. The DPI is written into the PNG metadata. The Esfera view (Plotly) calls `Plotly.toImage()` with explicit `width`/`height` (the logical canvas at 96 dpi; without them Plotly falls back to 700×450) and `scale = DPI/96`, raises `plotGlPixelRatio` so the WebGL mesh is sharp, and retries once before falling back to `QWebEngineView.grab()`; the result is read back over a chunked `console.log` relay (`EXPORTIMG:<id>:<i>:<n>:<chunk>`) captured by `_SilentPage.javaScriptConsoleMessage`, because `runJavaScript()` does not reliably resolve a Promise here. Superficie 3D's export is described above (`ui/gl3d_view.py`). The pyqtgraph views (Polar 2D/Espectro) are cloned off-screen at the canvas size (`export_pg_view`) and exported with `ImageExporter`; SVG is only offered for Polar 2D (own `QSvgGenerator` writer — pyqtgraph's `SVGExporter` crashes on this Qt, and the Espectro SVG doesn't clip its bars).
- Right-click on the Esfera WebGL canvas can't rely on Qt's native `customContextMenuRequested` (Plotly's orbit-camera controls swallow the browser's own `contextmenu` event) — it's relayed through the same console-log channel (`CONTEXTMENU:x,y`). Superficie 3D doesn't have this problem (see `ui/gl3d_view.py` above).
...
There is only one persistence format exposed in the GUI menu: **`.cclp`** (`core/session.py`, thin wrapper over `core/data_store.py`'s `save_results`/`load_results`, whose NPZ schema is documented at the top of that file). It's a lightweight session — computed directivity (global and per note), plot config, and `metadata['view']['session_ui']` (the *entire* `Bridge.state`, not just the Directividad controls, so reopening one restores every tab's controls exactly as they were) — with **no audio**. Since there's no raw tensor, a loaded `.cclp` cannot be reprocessed (no re-align/calibrate/re-detect-notes/recompute-with-other-bands); it's read-only for the 4 plots (`MainWindow._load_polar_npz_file`/`_save_polar_npz_file` repopulate the four plots, the note selector and the controls **without audio or a `MicArray`** — `TabDirectividad._npz` serves per-note data). To reprocess, reload the original WAV files. There is no "save raw audio tensor" feature in the GUI (deliberately removed). `save_results()` will use the first computed note as the "global" section if the user never computed directivity on "Todo el audio", so the file is always loadable even when only per-note directivity exists. `save_results()` opens the file handle itself before calling `np.savez_compressed` — passing a bare path string makes numpy silently force a `.npz` suffix (so a `.cclp` would get written as `foo.cclp.npz`). Note segmentation masks are separate: `.msk` JSON files via `TabNotas.save_mask`/`load_mask` (Herramientas ▸ Guardar/Cargar máscara…).
