...
def polar_nota(polar_alineado, nota, i_ref=10, sr=44100, notas=None,
               hop_length=512, umbral_confianza=0.5, tolerancia_cents=50):
    """
    Devuelve el tensor polar con el audio de solo la nota pedida.

    Copia el tensor completo y silencia (pone en cero) todos los frames donde
    pyin no detecta esa nota. La detección se hace sobre i_ref; el silenciado
    se aplica a todos los mics.

    Parámetros
    ----------
    polar_alineado   : np.ndarray 3D  (n_angulos x n_mics x n_samples)
    nota             : str            nombre de la nota (ej: 'Fa4')
    i_ref            : int            índice del mic para pyin (default: 10)
    sr               : int            sample rate (Hz)
    notas            : dict           {nombre: freq_Hz}. Default: NOTAS_FA_MAYOR
    hop_length       : int            hop length de pyin (default: 512)
    umbral_confianza : float          probabilidad mínima voiced por frame (default: 0.5)
    tolerancia_cents : float          margen en cents para aceptar un frame como esa nota (default: 50)

    Retorna
    -------
    audio_nota : np.ndarray float32  (n_angulos x n_mics x n_samples)
                 Audio real, silenciado donde pyin no detecta la nota.
    """
    if notas is None:
        notas = NOTAS_FA_MAYOR

    if nota not in notas:
        raise ValueError(f"Nota '{nota}' no encontrada. Disponibles: {list(notas.keys())}")

    f_objetivo = notas[nota]
    n_angulos, n_mics, n_samples = polar_alineado.shape

    audio_nota = polar_alineado.astype(np.float32)  # copia editable con audio real

    for i_az in range(n_angulos):
        signal = audio_nota[i_az, i_ref, :]
        ...
        f0_seguro  = np.where((f0 > 0) & ~np.isnan(f0), f0, 1.0)
        cents_diff = np.abs(1200 * np.log2(f0_seguro / f_objetivo))

        es_esta_nota = voiced_flag & (voiced_prob >= umbral_confianza) & (~np.isnan(f0)) & (cents_diff <= tolerancia_cents)

        # Silenciar sample a sample los frames que NO son esta nota
        no_es_nota = np.repeat(~es_esta_nota, hop_length)[:n_samples]
        audio_nota[i_az, :, no_es_nota] = 0.0
