...
Rediseño: tipografía con jerarquía (títulos / UI / datos mono), acento sólido
en botones primarios y colores semánticos desde la paleta. Reemplazo directo
del styles.py original — todos los #objectName se mantienen.

Tipografías (gratis, Google Fonts). Instalalas o bundlealas con QFontDatabase
para el look exacto; si no están, cae al stack del sistema:
  · Space Grotesk  → títulos
  · IBM Plex Sans  → UI
  · IBM Plex Mono / JetBrains Mono → datos / log
"""
from ui.theme import DARK

# Stacks con fallback del sistema (robustos si la fuente no está instalada)
_F_UI    = '"IBM Plex Sans", "Segoe UI", "Inter", "Ubuntu", sans-serif'
_F_TITLE = '"Space Grotesk", "IBM Plex Sans", "Segoe UI", sans-serif'
_F_MONO  = '"IBM Plex Mono", "JetBrains Mono", "Consolas", "Fira Mono", monospace'
...
def get_qss(p: dict) -> str:
    ...
    danger_bg  = "#3d1f2a" if dark else "#fdeaea"
    danger_bor = "#5a2a38" if dark else "#f0b4b4"
    danger_hov = "#4d2535" if dark else "#fbd6d6"
...
/* Acción primaria: acento sólido (sin gradiente) */
...
/* Acción primaria compacta del ribbon (mismos colores, menos padding) */
...
/* Chips del selector de banda (tercio de octava) */
QPushButton#band_chip {{
    background: {p['bg_dark']};
    color: {p['text2']};
    border: 1px solid {p['border']};
    border-radius: 7px;
...
QPushButton#band_chip:hover {{
    background: {p['bg_panel']};
    color: {p['text']};
    border-color: {p['border2']};
}}
QPushButton#band_chip:checked {{
    background: {p['accent']};
    color: {p['accent_ink']};
...
/* Campos de datos numéricos en monoespaciada (tabular) */
...
QLabel#label_ok    {{ color: {p['ok']};   font-weight: 600; }}
QLabel#label_warn  {{ color: {p['warn']}; font-weight: 600; }}
QLabel#label_error {{ color: {p['err']};  font-weight: 600; }}
